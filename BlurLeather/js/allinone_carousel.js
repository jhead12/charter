(function($) {
	...
	$.fn.allinone_carousel = function(options) {
		...
		return this.each(function() {
			...
			var bannerControls = $('<div class="bannerControls">   <div class="leftNav"></div>   <div class="rightNav"></div>    </div>  <div class="contentHolder"></div>   <div class="elementTitle"></div>	<div class="playOver"></div>');
			...
			allinone_carousel_playOver.css('top',parseInt(options.height-contentHolderUnit.height()-options.verticalAdjustment+($('#contentHolderUnit_'+current_obj.current_img_no, allinone_carousel_container).height()-allinone_carousel_playOver.height())/2)+'px');

			//rearange left wing
			current_obj.currentZ_index=100;
			for (m=1;m<=Math.floor(options.numberOfVisibleItems/2);m++){
				current_obj.currentZ_index--;
				//resizeDiv($('#contentHolderUnit_'+(total_images-m)),m,options,contentHolderUnitOrigHeight,current_obj);
				resizeDiv($('#contentHolderUnit_'+(total_images-m), allinone_carousel_container),m,options,contentHolderUnitOrigHeight,current_obj);
				$('#contentHolderUnit_'+(total_images-m), allinone_carousel_container).css('left',parseInt((options.width-contentHolderUnitOrigWidth)/2)-m*options.elementsHorizontalSpacing+'px');
				$('#contentHolderUnit_'+(total_images-m), allinone_carousel_container).css('top',parseInt(options.height-contentHolderUnitOrigHeight)-options.verticalAdjustment+m*options.elementsVerticalSpacing+'px');
				$('#contentHolderUnit_'+(total_images-m), allinone_carousel_container).css('z-index',current_obj.currentZ_index);
				$('#contentHolderUnit_'+(total_images-m), allinone_carousel_container).css('display','block');
			}
