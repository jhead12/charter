...
if($_REQUEST['action']=="delete"){
	if($obj->ValueExists("sub_category","category_id",$_REQUEST['category_id'])==true || $obj->ValueExists("products","category_id",$_REQUEST['category_id'])==true){
...
if(isset($_POST['btnupdate'])){
	if($obj->PreCategoryValueExists("category","category_title",$obj->ReplaceSql($_POST['category_title']),"category_id",$obj->ReplaceSql($_POST['category_id']),$obj->ReplaceSql($_POST['pre_category_id']))==false){
	$query="update category set category_title='".$obj->ReplaceSql($_POST['category_title'])."', pre_category_id='".$obj->ReplaceSql($_POST['pre_category_id'])."', category_image_approved='".$obj->ReplaceSql($_POST['category_image_approved'])."' where category_id='".$obj->ReplaceSql($_POST['category_id'])."'";
	$obj->UpdateQuery($query);
	if($_FILES['category_image']['name']!=''){
		$photo = array("name"=>$_FILES['category_image']['name'],"tmp_name"=>$_FILES['category_image']['tmp_name']);
		$obj->FixedUploadImage($photo,"category","category_image","category_id",$_POST['category_id'],$_SESSION['IMG_LARGE']['W'],$_SESSION['IMG_LARGE']['H']);
	}
	$_SESSION['ERRORTYPE'] = "success";
...
if(isset($_POST['btnsave'])){
	if($obj->PreCategoryValueExists("category","category_title",$obj->ReplaceSql($_POST['category_title']),"","",$obj->ReplaceSql($_POST['pre_category_id']))==false){	
	$query="insert into category set category_title='".$obj->ReplaceSql($_POST['category_title'])."', pre_category_id='".$obj->ReplaceSql($_POST['pre_category_id'])."', category_image_approved='".$obj->ReplaceSql($_POST['category_image_approved'])."'";
	$category_id=$obj->InsertQuery($query);
	$obj->UpdateQuery("update category set orderid = ".$category_id." where category_id = ".$category_id);
	if($_FILES['category_image']['name']!=''){
		$photo = array("name"=>$_FILES['category_image']['name'],"tmp_name"=>$_FILES['category_image']['tmp_name']);
		$obj->FixedUploadImage($photo,"category","category_image","category_id",$category_id,$_SESSION['IMG_LARGE']['W'],$_SESSION['IMG_LARGE']['H']);
	}
	$_SESSION['ERRORTYPE'] = "success";
...
if(isset($_POST['do_submit'])){
	...
	foreach($ids as $index=>$id) {
		$id = (int) $id;
		if($id != '') {
			$obj->UpdateQuery("update category set orderid = ".($index + 1)." where category_id = ".$id);
		}
	}
...
<div class="full"><h1>Manage <?php echo $obj->GetValue("pre_category","pre_category_title","pre_category_id=".$_REQUEST['pre_category_id']);?> Section Category
...
<ul class="tabs">
    ...
    <li><a href="?pre_category_id=<?php echo $_REQUEST['pre_category_id']?>" <?php echo $_GET['action']!='add' ? ' class="current"': '';?>>List of Category</a></li>
...
<table width="100%" cellspacing="1" cellpadding="10" class="tbl">
    ...
    <tr>
    	<td width="25%"><label id="err_category_title">Category Title : </label> <span class="error">*</span></td>
    ...
    <tr>
        ...
        <td><div class="full"><input type="file" name="category_image" id="category_image" class="isImg" title="Category Image"/></div>
            <?php if($obj->ImageExists("category",$data[0]['category_image'])){?>
            <div class="full paddtop10"><img src="../category/<?php echo $data[0]['category_image'];?>" height="113" width="159" /></div>
			<?php }?>
		</td>
	</tr>
     <tr>
...
<?php }elseif($_REQUEST['action']=="display"){
		$query = "select * from category where pre_category_id='".$_REQUEST['pre_category_id']."' order by orderid asc";
...
<table width="100%" cellspacing="1" cellpadding="10" class="tbl">
    ...
    <?php 
        ...
        $query="select * from category where pre_category_id='".$_REQUEST['pre_category_id']."' $where order by orderid asc";
	...
	<tr>
    	<td><?php echo $i++;?></td>
    	<td>
        <?php if($obj->ImageExists("category",$row['category_image'])){?>
        	<img src="../category/<?php echo $row['category_image'];?>" height="113" width="159"/>
        <?php }else{?>
        No Image
		<?php } ?>
        </td>
        <td><?php echo $row['category_title'];?></td>
        <td><?php echo $row['category_image_approved']=='1'?'Approved':'Not Approved';?></td>
        <td>
        	<a href="?action=edit&category_id=<?php echo $row['category_id'];?>&pre_category_id=<?php echo $row['pre_category_id'];?>" class="edit" title="Edit"></a>
            <a href="?action=delete&category_id=<?php echo $row['category_id'];?>&pre_category_id=<?php echo $row['pre_category_id'];?>" onclick="return confirm('Are you sure to delete?')" class="delete" title="Delete"></a>
		</td>            
	</tr>
        <?php } ?>
		<tr><td colspan="5" class="paging"><?php echo $pager->DisplayAllPaging("pre_categpory_id=".$_REQUEST['pre_categpory_id']."&alpha=".$alpha."&keyword=".$keyword);?></td></tr>
    <?php } else { ?>
