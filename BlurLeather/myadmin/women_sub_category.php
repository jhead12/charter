...
if($_REQUEST['action']=="delete"){
	if($obj->ValueExists("women_sub_sub_category","sub_category_id",$_REQUEST['sub_category_id'])==true){
		$_SESSION['ERRORTYPE'] = "error";
		$_SESSION['ERRORMSG'] = "Sub Category Could not be deleted! Already used in another forms";	
	}else{
		$query="delete from women_sub_category where sub_category_id='".$_REQUEST['sub_category_id']."'";
...
if(isset($_POST['btnupdate'])){
	if($obj->ValueExists("women_sub_category","sub_category_title",$obj->ReplaceSql($_POST['sub_category_title']),"sub_category_id",$obj->ReplaceSql($_POST['sub_category_id']))==false){
	$query="update women_sub_category set sub_category_title='".$obj->ReplaceSql($_POST['sub_category_title'])."', category_id='".$obj->ReplaceSql($_POST['category_id'])."' where sub_category_id='".$obj->ReplaceSql($_POST['sub_category_id'])."'";
	$obj->UpdateQuery($query);
	$_SESSION['ERRORTYPE'] = "success";
...
if(isset($_POST['btnsave'])){
	if($obj->ValueExists("women_sub_category","sub_category_title",$obj->ReplaceSql($_POST['sub_category_title']))==false){	
	$query="insert into women_sub_category set sub_category_title='".$obj->ReplaceSql($_POST['sub_category_title'])."', category_id='".$obj->ReplaceSql($_POST['category_id'])."'";
...
if(isset($_POST['do_submit'])){
	...
	foreach($ids as $index=>$id) {
		$id = (int) $id;
		if($id != '') {
			$obj->UpdateQuery("update women_sub_category set orderid = ".($index + 1)." where sub_category_id = ".$id);
		}
	}
...
<div class="full"><h1>Manage Women Sub Category<?php if($_REQUEST['action']=="edit"){ echo "[Edit]";} ?> <?php if($_REQUEST['action']=="add"){ echo "[Add]";} ?> <?php if($_REQUEST['action']=="view"){ echo "[View]";} ?></h1></div>
<ul class="tabs">
	<li><a href="?action=display"<?php echo $_GET['action']=='display' ? ' class="current"': '';?>>Display Order</a></li>
    <li><a href="?"<?php echo $_GET['action']!='add' ? ' class="current"': '';?>>List of Sub Category</a></li>
...
<?php 
    ...
    if($_REQUEST['action']=="edit" && isset($_REQUEST['sub_category_id'])){
        $query = "select * from women_sub_category where sub_category_id='".$_REQUEST['sub_category_id']."'";
    ...
    <tr>
        ...
        <td>
            <select id="category_id" name="category_id" class="R" title="Category">
                ...
                <?php if($category_cmb = $obj->SelectQuery("select * from women_category order by orderid")){?><?php foreach ($category_cmb as $category) { ?><option <?php echo ($data[0]['category_id']==$category['category_id'])?'selected="selected"':'';?> value="<?php echo $category['category_id']?>"><?php echo ucfirst($category['category_title']);?></option><?php }} ?>
            </select> 
        </td>
    </tr>
    <tr>
...
<?php }elseif($_REQUEST['action']=="display"){
		$query = "select * from women_sub_category order by orderid asc";
...
<table width="100%" cellspacing="1" cellpadding="10" class="tbl">
    ...
    <?php 
        ...
        $query="select * from women_sub_category as s inner join women_category as c on s.category_id=c.category_id where 1=1 $where order by s.orderid asc";
	...
	<tr>
        ...
        <td>
        	<a href="?action=edit&sub_category_id=<?php echo $row['sub_category_id'] ?>" class="edit" title="Edit"></a>
            <a href="?action=delete&sub_category_id=<?php echo $row['sub_category_id'] ?>" onclick="return confirm('Are you sure to delete?')" class="delete" title="Delete"></a>
		</td>            
	</tr>
        <?php } ?>
		<tr><td colspan="4" class="paging"><?php echo $pager->DisplayAllPaging();?></td></tr>
    <?php } else { ?>
