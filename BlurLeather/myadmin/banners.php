<?php require_once("../class/class.admin.php");
	...
	if(isset($_POST['btnupdate']) || isset($_POST['btnsave'])){
		...
		if($_FILES['_img']['name']!=""){
			$obj->FixedUploadImage($_FILES['_img'],"banners","banner_image","banner_id",$banner_id,1980,1080,339,215,true);
		}
		$obj->ReturnReferer();
	}
	if(isset($_POST['do_submit']))  {
	  	...
	  	foreach($ids as $index=>$id) {
    		$id = (int) $id;
    		if($id != '') {
    	  		$obj->UpdateQuery("update banners set orderid = ".($index + 1)." where banner_id = ".$id);
    		}
  		}
  		if($_POST['byajax']) { die(); } else { $message = 'Sortation has been saved.'; }
	}
	if($_REQUEST['T']=='D'){
		$data = $obj->SelectQuery("select banner_image from banners where banner_id='".$_REQUEST['banner_id']."'");
		@unlink("../banners/".$data[0]['banner_image']);
		@unlink("../banners/th_".$data[0]['banner_image']);
		$query="delete from banners where banner_id='".$_REQUEST['banner_id']."'";
		$obj->UpdateQuery($query);
		$_SESSION['ERRORTYPE'] = "success";
...
<body>
<div class="full">
<h1>Manage <?php echo $obj->GetValue("pre_category","pre_category_title","pre_category_id=".$_REQUEST['pre_category_id']);?> Banners <?php if($_REQUEST['action']=="edit"){ echo "[Edit]";} ?> <?php if($_REQUEST['action']=="add"){ echo "[Add]";} ?> <?php if($_REQUEST['action']=="view"){ echo "[View]";} ?></h1>
</div>
<?php require_once("message.php");?>
	<?php if($_REQUEST['action']=="add" || $_REQUEST['action']=="edit"){
			if($_REQUEST['action']=="edit" && isset($_REQUEST['banner_id'])){
				$query = "select * from banners where banner_id='".$_REQUEST['banner_id']."'";
        ...
        <tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="full paddtop10"><img src="../banners/th_<?php echo $data[0]['banner_image'];?>" /></div>
					<?php }?>
                </td>
			</tr>
			<tr>
    ...
    <?php }else{ $obj->SetCurrentUrl();
		$query = "select * from banners where pre_category_id='".$_REQUEST['pre_category_id']."' order by orderid asc";
...
<?php if($result){?>
	...
	<?php $order = array();
		foreach($result as $item) {
			echo '<li title="'.$item['banner_id'].'"><a href="javascript:deleteconfirm(\''.$item['banner_id'].'\');" title="Delete" class="delete"></a>&nbsp;<a href="?action=edit&banner_id='.$item['banner_id'].'&pre_category_id='.$item['pre_category_id'].'" title="Edit" class="edit"></a><br /><br /><img src="../banners/th_'.$item['banner_image'].'" height="90" /></li>';
