...
if($_REQUEST['action']=='delete'){
	$query="delete from class_order where class_order_id='".$obj->ReplaceSql($_REQUEST['class_order_id'])."'";
	$obj->UpdateQuery($query);
	$_SESSION['ERRORTYPE'] = "success";
	$_SESSION['ERRORMSG'] = "Classes Booking Order has been deleted successfully!";
	$obj->ReturnReferer();
	exit();
}
?>
...
<h1>Manage Classes Booking Orders <?php if($_REQUEST['action']=="edit"){ echo "[Edit]";} ?> <?php if($_REQUEST['action']=="add"){ echo "[Add]";} ?> <?php if($_REQUEST['action']=="view"){ echo "[View]";} ?></h1>
<ul class="tabs">
...
<table width="100%" cellspacing="1" cellpadding="3" class="tbl">
    <?php 
           $query = "select * from class_order as s left outer join payments as mp on s.class_order_id=mp.paypal_order_id order by s.class_order_id desc";
            ...
            if($orders = $pager->Paging()){$sno = $pager->GetSNo();?>
                ...
                <?php foreach($orders as $order){?>
                 <tr>
                    <td><?php echo $order['class_order_id'];?></td>
                    <td><?php echo date("M d, Y",strtotime($order['order_date'])); ?></td>
                    <td><?php echo $order['class_title'];?></td>
                    <td><?php echo $order['full_name'];?></td>
                    <td><?php echo $order['email'];?></td>
                    <td><?php echo $order['class_price'];?></td>
                    <td><?php echo $order['city'];?>, <?php echo $order['state'];?>, <br /><?php echo $order['country'];?>, <br /><?php echo $order['mobile_no'];?></td>
                    <td id="payment_status<?php echo $order['class_order_id'];?>">
                    	<?php if($order['payment_status']=="Y") { 
							echo "<a href=\"javascript:;\" onclick=\"senddata('orders','type=class&val=N&class_order_id=". $order['class_order_id']."','payment_status".$order['class_order_id']."');\">Pending</a> | <strong>Paid</strong>";
						}elseif($order['payment_status']=="N") {
							echo "<strong>Pending</strong> | <a href=\"javascript:;\" onclick=\"senddata('orders','type=class&val=Y&class_order_id=".$order['class_order_id']."','payment_status".$order['class_order_id']."');\">Paid</a>";
						}?>
                    </td>
                    <td>
                    	<a class="view" href="javascript:;" onclick="$('#order_details<?php echo $order['class_order_id'];?>').slideToggle(500);"></a>
                        <a class="delete" onclick="return confirm('Are you sure to delete?');" href="?action=delete&class_order_id=<?php echo $order['class_order_id'];?>"></a>
                    </td>
                </tr>
                <tr>
                	<td colspan="9">
                    	<div class="full none" id="order_details<?php echo $order['class_order_id'];?>">
